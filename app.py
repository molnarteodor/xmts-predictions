...
def generate_algorithmic_prediction(match, seed_offset=0):
    ...
    # Algoritm determinist bazat pe hash-ul meciului și numele echipelor
    ...
    # Determinare dinamică a stilului de joc
    ...
    if is_high_scoring:
        ...
        main_pred = f"1X & Peste 1.5 Goluri"
    ...
    bb_options = [
        ...
        {
            "label": "🔥 BetBuilder Cotă Mare (Cotă ~4.10)",
            "selection": f"{match['home_team']} Victorie sau Egal + Peste 2.5 Goluri + Peste 8.5 Cornere",
...
<head>
    ...
    <style>
        ...
        .search-input, .date-input { background: #0f172a; color: #fff; border: 1px solid #475569; padding: 10px 12px; border-radius: 6px; font-size: 0.9rem; }
        ...
        .bb-box { background: #0f172a; border: 1px dashed #0284c7; border-radius: 8px; padding: 10px; margin-top: 8px; display: block; }
...
<body>
    <div class="container">
        ...
        <div class="nav-tabs">
            <button class="tab-btn active" id="btn-matches" onclick="switchTab('matches')">Meciuri Toate</button>
            <button class="tab-btn" id="btn-popular" onclick="switchTab('popular')">🔥 Cele Mai Jucate</button>
            <button class="tab-btn" id="btn-tickets" onclick="switchTab('tickets')">⭐ Bilete Top</button>
            <button class="tab-btn" id="btn-chat" onclick="switchTab('chat')">💬 AI Chat</button>
        </div>
        ...
        <div id="tab-tickets" style="display:none;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
                ...
                <button class="btn-action btn-regen" onclick="loadTickets(true)">🔄 Regenerază Bilete</button>
            </div>
        ...
        <div id="tab-chat" style="display:none;">
            <div class="chat-box" id="chat-messages">
                <div class="chat-msg chat-ai">Salut! Trimite imagini sau întrebări legate de meciuri.</div>
            </div>
    ...
    <script>
        ...
        function filterMatches() {
            ...
            if (filtered.length === 0) {
                container.innerHTML = '<p style="text-align:center; color:#94a3b8;">Nu s-au găsit meciuri pentru selecția curentă.</p>';
                return;
            }
            ...
            filtered.forEach(m => {
                ...
                card.innerHTML = `
                    <div>
                        <div class="match-league">${m.league} ${m.is_popular ? '<span style="color:#eab308;">🔥 Cele Mai Jucate</span>' : ''}</div>
                    ...
                    <div class="action-bar">
                        <button class="btn-action btn-regen" onclick="regeneratePrediction(${m.id})">🔄 Regenerază Pronostic</button>
                    </div>
                    ` : ''}
                    ${bbHtml}
                `;
            ...
            fetch(`/api/tickets?date=${dateInput.value}&seed3=${ticketSeeds[3]}&seed5=${ticketSeeds[5]}&seed7=${ticketSeeds[7]}`)
                ...
                .then(data => {
                    ...
                    if (data.length === 0 || data[0].matches.length === 0) {
                        container.innerHTML = '<p style="text-align:center; color:#94a3b8;">Nu există meciuri suficiente pentru această dată.</p>';
                        return;
                    }
                    data.forEach(t => {
